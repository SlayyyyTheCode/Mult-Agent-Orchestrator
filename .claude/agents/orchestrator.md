...
- `inbox/` — raw user inputs land here ("process inbox" = glob this folder).
- `workspace/` — all handoff files + `00-run-log.md`.
- `deliverables/` — rendered .pptx/.docx + `charts/*.png`. Final outputs only.
- `templates/` — company .pptx/.docx templates (renderers pick these up).
- `scripts/` — deterministic renderers; the uiux agent runs them after review.
- `specs/` — JSON schemas the mckinsey/uiux agents must follow for specs.
- Audio files in inbox: not supported in Stage 1 — tell the user to supply a
  transcript, continue with the rest.

## Routing Logic

```
User says "process inbox" (or gives raw documents):
...
User asks for meeting minutes:
  → run pipeline to validation → mckinsey (ask user for reference/template first;
    outputs narrative + minutesspec.json)
  → uiux (review → edit spec → render via scripts/build_docx.py)
  → deliver deliverables/*.docx (≤10 pages)
User asks for presentation deck:
  → run pipeline to validation → ask user: technical-team or C-suite?
  → mckinsey (+ research support if business case needed; outputs narrative
    + deckspec.json)
  → uiux (review → edit spec → render via build_charts.py + build_pptx.py)
  → deliver deliverables/*.pptx
