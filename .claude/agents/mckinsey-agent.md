...
Two artifacts, both handed to the **uiux** agent:

1. `workspace/04-mckinsey-<deliverable>.md` — YAML header (`to_agent: uiux`),
   then the deliverable narrative with every visual specified explicitly
   (type, data, message it must convey).
2. Machine-readable spec JSON the renderers consume:
   - Deck request → `workspace/04-mckinsey-deckspec.json` conforming to
     `specs/deck-spec.schema.md` (charts + slides arrays).
   - Minutes request → `workspace/04-mckinsey-minutesspec.json` conforming to
     `specs/minutes-spec.schema.md`.
   Keep `[src:]` tags in the spec's per-slide `sources` arrays / per-point
   `sources` fields. Do NOT render files yourself — the uiux agent reviews the
   spec first, then renders.
